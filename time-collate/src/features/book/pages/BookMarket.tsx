...
export function BookMarket({ isEmbed = false }: { isEmbed?: boolean }) {
        ...
        <div className="flex flex-col gap-8 p-8 max-w-7xl mx-auto font-['Outfit',_sans-serif] min-h-screen">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="relative w-full sm:w-72">
                        ...
                        <input
                            ...
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[12px] text-slate-700
                                     placeholder-slate-400 focus:outline-none focus:bg-white focus:border-indigo-500 transition-all duration-300"
                        />
                    </div>
                </div>
            ...
            {isLoading ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center py-24 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
                    <BookOpen size={48} className="text-slate-350 mb-3" />
                    <h3 className="text-sm font-black text-slate-700">暂无相关书模板</h3>
                    <p className="text-slate-400 text-xs mt-1">换个分类或搜索词试试吧</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {filteredTemplates.map(tpl => (
                        <div
                            key={tpl.id}
                            className="group bg-white rounded-3xl border border-slate-200/50 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col"
                        >
                            {/* 封面预览图 */}
                            ...
                            <div className="p-5 flex-1 flex flex-col gap-4">
                                ...
                                <div className="mt-auto pt-2">
                                    <button
                                        ...
                                        className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-indigo-50 text-indigo-650 rounded-2xl
                    ...
                    <div className="bg-white w-full max-w-5xl rounded-[32px] overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                            ...
                            <button
                                ...
                                className="p-2.5 bg-slate-50 hover:bg-slate-100 text-slate-400 hover:text-slate-650 rounded-full transition-colors cursor-pointer"
                            >
                                <X size={20} />
                            </button>
                        </div>
    ...
    return <MainLayout title="书模板市场">{content}</MainLayout>;
}
